...
const UserProvider = ({ children }) => {
    ...
    try {
      ...
      if (result.data.user.role === "STAFF") {
        ...
        if (pendingCode) {
          ...
          navigate(`/validate-ticket?code=${pendingCode}`)
        } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // *** AGGIUNGI LA LOGICA DI NAVIGAZIONE QUI ***
      if (loggedInUser.role === "STAFF") {
        // Logica specifica per staff (potrebbe reindirizzare altrove)
         const pendingCode = localStorage.getItem("pendingTicketCode")
         if (pendingCode) {
           localStorage.removeItem("pendingTicketCode")
           navigate(`/validate-ticket?code=${pendingCode}`)
         } else {
           navigate("/validate-ticket") // Esempio: reindirizza a pagina validazione per staff
         }
      } else {
        // Logica per utenti normali (o altri ruoli) - reindirizza alla home
        navigate("/");
      }
      // *** FINE LOGICA DI NAVIGAZIONE ***
